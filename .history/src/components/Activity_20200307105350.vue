  ...
  <el-card class="activityCard" >
    ...
    <div class="activityAction" >
      ...
      <el-dialog
        ...
        class="testsCard"
        >
        <template v-for="(item,index) in testRecords" :key="index">
          <el-button type="success" plain class="testButton" @click="joinTest(item)" >{{item.thisTestName}}</el-button>
        </template>
...
export default {
  ...
  joinTest(item){
    ...
    StudentAddTestRecord(studentData).then(res=>{
      // console.log(res.data)
      // alert('加入成功')
