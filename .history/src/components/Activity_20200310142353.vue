    ...
    <div class="activityAction" >
      ...
      <el-dialog
        ...
        <template v-for="(item,index) in testRecords" >
          <el-button :type=" item.nowTest ? 'success':'primary' " plain class="testButton" @click="joinTest(item)" :key="index">{{item.thisTestName}}</el-button>
        </template>
...
export default {
  ...
  methods: {
      ...
      this.testFormData.thisTestName = this.testRecord.createTime + ' ' + this.activities.title
       ...
       GetTestRecords(this.testFormData._id).then(res=>{
         ...
         res.data.forEach(e => {
           ...
           e.endTime = moment(e.endTime).format('MM-DD HH:mm'); 
          
            let nowTime = moment().format('MM-DD HH:mm'); 
           if(moment(e.endTime ).isBefore(nowTime)){
              //在测验结束时间            
              this.testRecords.nowTest = false;
              
            }else{
              //还在测验              
               this.testRecords.nowTest = true;
   ...
   createTestRecord(){
      ...
      this.testRecord.endTime = moment(this.testRecord.createTime).add(this.testRecord.duration, 'minutes').format('YYYY-MM-DD HH:mm'); 
  ...
  joinTest(item){
    ...
    StudentAddTestRecord(studentData.testRecordID,studentData).then(res=>{
