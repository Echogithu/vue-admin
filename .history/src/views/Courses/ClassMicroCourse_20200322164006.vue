      ...
      <el-col :span="15">
        <div class="grid-content ">
          <el-card class="activityLists">
            <el-button type="text" :class="activityType=='all'? 'active' : ''" @click="choose(type='all')">全部活动</el-button>
            <el-button type="text" :class="activityType=='microcourses'? 'active' : ''" @click="choose(type='microcourses')">微课</el-button>
            <el-button type="text"  :class="activityType=='showtest'? 'active' : ''" @click="choose(type='showtest')">测验</el-button>
        ...
        <!-- 微课列表 -->
        <div  v-show="activityType=='microcourses'||activityType=='all'">
          <template v-for="(item,index) in microCourses"> 
            <MicroCourse :microCourse="item" :role="role" :userInfo="userInfo" :key="index"></MicroCourse>
          </template>
        </div>
        <!-- 测验列表 -->
        <div  v-show="activityType=='showtest'||activityType=='all'">
...
export default {
  components:{Group, Activity, MicroCourse},
  ...
  methods: {
    fetch(){
      ...
      /***获取活动-微课 */
      GetMicroCourse(this.$route.params.id).then(res=>{
        this.microCourses = res.data
        for(let i = 0;i<this.microCourses.length;i++){
          this.microCourses[i].time = moment(this.microCourses[i].createTime).format('YYYY-MM-DD HH:mm'); 
        }
        console.log('weikle')
        console.log(this.microCourses)
      })
