  ...
  <div class="testContent">
    <!-- <el-container> -->
      ...
      <div class="time">
        ...
        <span>{{testRecord.createTime}}~{{testRecord.createTime}}</span>
      </div>
      ...
      <div class="main"> 
        ...
        <div class="mainContent"  >
          <template v-for="(item, index) in testArrays">
            <span :key="index" class="topics" :class="studentAnswer.chooseAnswers[index] !== undefined ? 'selected':''" >{{index+1}}</span>
          </template>
        </div>
  ...
  methods: {
    fetch(){
      ...
      GetTestRecord(this.studentData).then(res=>{
        ...
        this.testRecord.endTime =  moment(this.testRecord.endTime).format('MM-DD HH:mm'); 
        // this.endTime = moment(this.testRecord.createTime).add(this.testRecord.duration, 'minutes').format('MM-DD HH:mm'); 

        //  console.log('123')
        // console.log(this.testRecord.endTime)
      })
    ...
    //学生提交答案
    submitStudentAnswer(){
      this.studentAnswer.studentID = this.testRecord.studentsID  
      console.log(this.testArrays)
      for(let i =0; i < this.testArrays.length; i++){
        // console.log(this.studentAnswer.chooseAnswers[i] == undefined)
        //如果没有选择选项，则添加为0
        if(this.studentAnswer.chooseAnswers[i] == undefined){
          this.studentAnswer.chooseAnswers[i] = 0
        }
      }      

      //第一次提交成绩
      PostTestRecord(this.testRecordID,this.studentAnswer).then(res=>{
        //跳转到结果页面--显示成绩
        this.$router.push(
          { 
            path:'/test/results', 
            query: {
              testID: this.testRecord.testID,
              testRecordID: this.testRecordID,  
              studentID:  this.testRecord.studentsID,
            }
          }
        )
      })
